...
## September 17 v0.2.0 release preparation

The user explicitly authorized publication of all current source and Docker improvements as **Command Table v0.2.0**. This release includes the rebrand, fresh-game/rematch flow, automatic mobile layouts, group life effects, commander rulings, recap PNGs, player-status badges and Docker-first hosting instructions. The earlier local-only entries below are historical preview records. Publication is being prepared; no successful remote release or image publication is claimed yet.

Local ESLint, TypeScript, the production build and **172 unit/integration checks across ten files** pass. The complete production-Docker browser coverage passes **130 unique cases across 19 files, with two expected WebKit skips** (132 total). Two test fixtures were corrected: waiting for a durable save before reload, and blocking service workers in contexts that deliberately mock artwork. The corrected artwork rerun passed 8/8 and the final screenshot/layout rerun passed 2/2; no production defect or unresolved browser failure remains.

Production PWA prompted-update/offline checks pass in Chromium and WebKit, along with HTTPS/WSS proxy and LAN checks. The LAN expectation now reflects a fresh four-seat, 40-life shared room and 39 life after a decrement. `npm audit` reports zero known vulnerabilities at all severities. Published/local Compose configurations, actionlint and five mocked workflow cases covering stable/prerelease releases, drafts, reruns and upload failure pass. The v0.2.0 candidate image `f4c427d1743b` passed the isolated x64 container smoke test, and its temporary container was removed.

Remote CI, native arm64 release checks and publication are still pending. The release workflow will repeat the full browser, PWA and proxy gate and verify native amd64 and arm64 containers before publishing.
